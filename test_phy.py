...
# ==============================================================================
MONTE_CARLO_RUNS = 10000  # Valor base para las pruebas de Monte Carlo (10000 para rapidez)
...
GI_SELECCIONADO = "0.8us"  # Opciones: "0.8us", "1.6us", "3.2us"

# Parámetros físicos del estándar IEEE 802.11bb (HE-PHY 20 MHz SISO con Frecuencia Intermedia)
FFT_SIZE = 1024
ACTIVE_CARRIERS = list(range(139, 256)) + list(range(257, 374))  # 234 portadoras de datos centradas en FI = 20 MHz (canal 256)
# ==============================================================================
...
def test_hermitian_symmetry():
    ...
    tx_symbols = lifi_phy.constellation_mapping(tx_bits, scheme="QPSK")
    
    # Aplicar simetría
    ofdm_freq = lifi_phy.apply_hermitian_symmetry(tx_symbols, fft_size=FFT_SIZE, active_carriers=ACTIVE_CARRIERS)
...
def test_ideal_channel():
    ...
    for gi_name, cp_len in gis.items():
        for mcs in range(12):
            ...
            # Canal Ideal
            s_rx_opt = lifi_phy.optical_wireless_channel(
                s_opt, distance=1.0, rx_area=2*np.pi/(2), semi_angle_led=60.0, 
                fov_rx=60.0, index_conc=1.0, noise_variance=0.0
            )
            
            # Cadena RX
            i_rx = lifi_phy.rx_optical_front_end(s_rx_opt, responsivity=1.0)
            ...
            rx_punctured = lifi_phy.deinterleave(rx_interleaved)
            ...
            rx_coded, is_erasure = lifi_phy.depuncture(rx_punctured_truncated, rate=rate, target_len=len(coded))
...
def generate_ber_curves(solar_irradiance: float = 150.0, 
    ...
    index_conc = 1.5
    responsivity = 0.6
    bandwidth = 40e6  # Ancho de banda de ruido
    
    # Varianza del ruido eléctrico solar de fondo
    phi_half = np.radians(semi_angle)
    m = -np.log(2.0) / np.log(np.cos(phi_half))
    g_psi = (index_conc**2) / (np.sin(np.radians(fov_rx))**2)
    P_sol = solar_irradiance * 0.01 * rx_area * 1.0 * g_psi
    ...
    for mcs in mcs_selection:
        ...
        for dist in distance_range:
            ...
            s_rx_optical = lifi_phy.optical_wireless_channel(
                s_opt, distance=dist, rx_area=rx_area, semi_angle_led=semi_angle,
            ...
            H_0 = ((m + 1) * rx_area / (2 * np.pi * (dist**2))) * g_psi
...
def generate_scenario_curves(solar_noise: bool = True, filename: str = "curvas_rendimiento_escenarios.png"):
    ...
    mcs_ref = 1
    mod, rate = lifi_phy.get_mcs_params(mcs_ref)
    ...
    for name, solar_irradiance, extinction_coef, color, marker in scenarios:
        ...
        for dist in distance_range:
            ...
            # Lambertiana + Beer-Lambert para ecualizar
            phi_half = np.radians(semi_angle)
            m = -np.log(2.0) / np.log(np.cos(phi_half))
            g_psi = (index_conc**2) / (np.sin(np.radians(fov_rx))**2)
            H_0 = ((m + 1) * rx_area / (2 * np.pi * (dist**2))) * g_psi
            atmospheric_attenuation = np.exp(-extinction_coef * dist)
